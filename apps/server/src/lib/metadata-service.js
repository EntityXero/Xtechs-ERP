 ...
 */
export async function resolveMetadata(db, key, context) {
    ...
    conditions.push(and(isNull(metadataRevisions.tenantId), isNull(metadataRevisions.businessId), isNull(metadataRevisions.branchId)));
    if (tenantId) {
        // Tenant scope
        conditions.push(and(eq(metadataRevisions.tenantId, tenantId), isNull(metadataRevisions.businessId), isNull(metadataRevisions.branchId)));
        if (businessId) {
            // Business scope
            conditions.push(and(eq(metadataRevisions.tenantId, tenantId), eq(metadataRevisions.businessId, businessId), isNull(metadataRevisions.branchId)));
            if (branchId) {
                // Branch scope
                conditions.push(and(eq(metadataRevisions.tenantId, tenantId), eq(metadataRevisions.businessId, businessId), eq(metadataRevisions.branchId, branchId)));
            }
        }
    }
    ...
    const revs = await db.select()
        ...
        .where(and(eq(metadataRevisions.defId, def.id), or(...conditions)))
        .orderBy(desc(metadataRevisions.version));
    if (revs.length === 0)
        return { ...def, revision: null };
    const findRevision = (scopeType) => {
        return revs.find((r) => {
            if (scopeType === 'branch') {
    ...
    if (branchRev)
        return { ...def, revision: branchRev };
    const businessRev = businessId ? findRevision('business') : null;
    if (businessRev)
        return { ...def, revision: businessRev };
    const tenantRev = tenantId ? findRevision('tenant') : null;
    if (tenantRev)
        return { ...def, revision: tenantRev };
    const globalRev = findRevision('global');
    if (globalRev)
        return { ...def, revision: globalRev };
    return { ...def, revision: null };
}
/**
 * Creates a new metadata definition.
 */
export async function createMetadataDefinition(db, input) {
 ...
 */
export async function createMetadataRevision(db, key, input) {
