...
export default function PurchasingPage() {
  const { data, isLoading, isError, refetch } = useDocuments('purchase_order');

  const purchaseOrders = data?.data ?? [];
  const total = data?.total ?? 0;
  const postedCount = purchaseOrders.filter((doc) => doc.status === 'POSTED').length;
  const draftCount = purchaseOrders.filter((doc) => doc.status === 'DRAFT').length;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between border-b border-border/60 pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5 font-mono">
            Track suppliers, issue purchase orders, and record goods receipts.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button asChild size="sm" className="h-8 text-xs gap-1.5">
            <Link href="/documents/purchase_order/new">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 select-none">
        <div className="rounded-md border border-border bg-card p-4">
          ...
          <p className="text-[10px] text-muted-foreground mt-1">Total purchase orders issued</p>
        </div>

        <div className="rounded-md border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Posted POs</span>
          ...
          <p className="text-[10px] text-muted-foreground mt-1">POs finalized and posted to inventory</p>
        </div>

        <div className="rounded-md border border-border bg-card p-4">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Draft POs</span>
          ...
          <p className="text-[10px] text-muted-foreground mt-1">POs pending supplier confirmation</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between border-b border-border/60 pb-2">
          <h2 className="text-sm font-bold tracking-tight text-foreground font-sans">Recent Purchase Orders</h2>
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="text-xs text-muted-foreground font-mono">Loading purchase orders...</span>
          </div>
        )}
